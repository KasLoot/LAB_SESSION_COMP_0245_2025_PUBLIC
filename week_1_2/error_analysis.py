...
def check_ill_conditioning(Y, experiment_name=""):
    ...
    print("Difference from identity (Frobenius norm):", np.linalg.norm(I_est - I, 'fro')) 
...
def run_all_regression_methods(Y, tau_mixed, tau):
    ...
    print("\n--- 方案 3：正则化伪逆（Damped Least Squares） ---")
    alpha = 1e-3  # 可调参数
    a_est = np.linalg.solve(Y.T @ Y + alpha * np.eye(Y.shape[1]), Y.T @ tau_compute)
...
def main():
    ...
    tau_mixed = np.hstack((tau_real, tau_mes_all[:,6].reshape(-1,1))).reshape(-1)
