"""
Training/evaluation script for Part 2.
"""
import os
...
CONFIG = {
    ...
    "plot": {
        "loss_curve_path": "part2_with_stop_loss_curves.png",
    },
}
...
class P2_dataset(Dataset):
    """Load concatenated .pt trajectory files and provide (q_mes, desired_cart_pos, q_des, qd_des)."""
    ...
    def _load_files(self):
        ...
        for f in files:
            ...
            # Expect keys: q_mes_all, final_cartesian_pos, q_d_all, qd_d_all
            self.q_mes_all.append(np.asarray(data.get("q_mes_all", [])))
            self.desired_cartesian_pos_all.append(np.asarray(data.get("final_cartesian_pos", [])))
            self.q_des_all.append(np.asarray(data.get("q_d_all", [])))
            self.qd_des_all.append(np.asarray(data.get("qd_d_all", [])))

        if len(self.q_mes_all) == 0:
            # Empty dataset guard
            self.q_mes_all = torch.tensor([], dtype=DTYPE)
            self.desired_cartesian_pos_all = torch.tensor([], dtype=DTYPE)
            self.q_des_all = torch.tensor([], dtype=DTYPE)
            self.qd_des_all = torch.tensor([], dtype=DTYPE)
            return

        # Concatenate and convert to tensors
        self.q_mes_all = torch.tensor(np.concatenate(self.q_mes_all, axis=0), dtype=DTYPE)
        self.desired_cartesian_pos_all = torch.tensor(np.concatenate(self.desired_cartesian_pos_all, axis=0), dtype=DTYPE)
        self.q_des_all = torch.tensor(np.concatenate(self.q_des_all, axis=0), dtype=DTYPE)
        self.qd_des_all = torch.tensor(np.concatenate(self.qd_des_all, axis=0), dtype=DTYPE)
...
class P2_MLP(nn.Module):
    """Simple MLP for mapping inputs -> outputs. Hidden sizes configured via CONFIG."""
...
def plot_loss_curves(train_losses, val_losses, test_losses, save_path=None):
    """Plot and save train/val/test loss curves."""
...
# -------------------------
def train():
    """Train the model using CONFIG hyperparameters."""
    device = get_device()
    ...
    train_dataset = P2_dataset(train_dir)
    ...
    test_dataset = P2_dataset(test_dir)
    test_loader = DataLoader(test_dataset, batch_size=eval_batch_size, shuffle=False)

    # Model / optimizer / loss
    model = P2_MLP(input_size=CONFIG["model"]["input_size"], output_size=CONFIG["model"]["output_size"], hidden=CONFIG["model"]["hidden"], activation=CONFIG["model"]["activation"])
    ...
    for epoch in range(num_epochs):
        ...
        for batch in tqdm(train_loader, desc=f"Epoch {epoch+1}/{num_epochs} - train"):
            ...
            inputs = torch.cat((q_mes, desired_cartesian_pos), dim=1).to(device=device, dtype=DTYPE)
            targets = torch.cat((q_des, qd_des), dim=1).to(device=device, dtype=DTYPE)
        ...
        with torch.no_grad():
            ...
            avg_val_loss = float(np.mean(val_batch_losses)) if len(val_batch_losses) else float("nan")
    ...
    # Plot loss curves
    plot_loss_curves(train_losses, val_losses, test_losses, save_path=CONFIG["plot"]["loss_curve_path"])
...
def evaluate_best_model(model_path: str = None, data_dir: str = None, batch_size: int = None, device: torch.device = None):
    """
    Evaluate saved model on dataset and return per-dimension MSE and R^2 plus overall metrics.
    Uses only numpy/torch (no sklearn).
    """
    ...
    preds = np.concatenate(preds_list, axis=0) if len(preds_list) else np.empty((0, CONFIG["model"]["output_size"]))
    targets = np.concatenate(targets_list, axis=0) if len(targets_list) else np.empty((0, CONFIG["model"]["output_size"]))
