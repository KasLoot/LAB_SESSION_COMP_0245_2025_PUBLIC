...
def collect_data(num_poses=5, save_path="q_diff.pt"):
    ...
    for i in range(len(list_of_desired_cartesian_positions)):
        ...
        for t in range(steps):
            ...
            prediction = model(torch.tensor(q_des-q_mes).to(torch.float32).to(torch.device('cuda')))
            tau_cmd_model = prediction.cpu().detach().numpy()
            print(f"NN model tau_cmd: {tau_cmd_model}")

            cmd.SetControlCmd(tau_cmd_model, ["torque"] * 7)  # Set the torque command
