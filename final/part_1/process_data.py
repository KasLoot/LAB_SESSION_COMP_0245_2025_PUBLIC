...
class Part_1_Dataset(Dataset):
    def __init__(self, data_path):
        ...
        print(f"tau_mes[-1]: {self.tau_mes[-1]}")
...
def process_transformer_data(data_path, slide_window=5):
    q_mes_all = []
    qd_mes_all = []
    q_d_all = []
    qd_d_all = []
...
class Part_1_Transformer_Dataset(Dataset):
    def __init__(self, data_path):
        self.q_mes, self.qd_mes, self.q_d, self.qd_d, self.tau_mes, self.cart_pos, self.cart_ori = process_data(data_path=data_path)
        ...
        self.q_diff = self.q_diff.to(torch.float32)
        self.qd_diff = self.qd_diff.to(torch.float32)
        self.tau_mes = self.tau_mes.to(torch.float32)
